...
public class STDFReader {
    @Data
    private static class MyType implements Comparable {
        int type;
        int subType;

        public MyType(int type, int subType) {
            this.type = type;
            this.subType = subType;
        }

        public int compareTo(Object o) {
            return (this.getType() == ((MyType) o).getType()) ?
                    this.subType - ((MyType) o).getSubType() : this.getType() - ((MyType) o).getType();
        }
    }

    private static List<MyType> undefinedTypes = new LinkedList<MyType>();
    ...
    public void parse(RecordVisitor visitor, Map<RecordType, RecordDescriptor> records)
        ...
        try {
            ...
            try {
                cpuType = (Long) far.getField("CPU_TYPE");
            } catch (Exception e) {
                //ignore
                e.printStackTrace();
            }
            ...
            Map<MyType, Integer> counter = new TreeMap<MyType, Integer>();
            for (MyType a : undefinedTypes) {
                if (counter.containsKey(a)) {
                    int tmp = counter.get(a);
                    counter.put(a, tmp + 1);
                } else {
                    counter.put(a, 1);
                }
            }
            for (Map.Entry<MyType, Integer> a : counter.entrySet()) {
    ...
    protected Record readRecord(Header header,
        ...
        if (records.containsKey(header.getRecordType())) {
            ...
            if (errorOnUnknown) {
                ...
                undefinedTypes.add(new MyType(header.getType(), header.getSubType()));
                getBytes(header.getLength());
            }
        }
